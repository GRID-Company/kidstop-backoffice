...
type ApiInventoryItem = NonNullable<
  NonNullable<InventoryItemsQuery['inventoryItems']['data']>[number]
>;
type ApiInventoryMovement = NonNullable<
  NonNullable<InventoryMovementsQuery['inventoryMovements']['data']>[number]
>;
...
export function fromApiInventoryItem(item: ApiInventoryItem): IInventoryItem {
  ...
  return {
    ...
    number:
      ...
      '',
    rarity:
      item.pokemonCardSummary?.rarity ?? item.magicCardSummary?.rarity ?? '',
    ...
    stock: item.stock,
    stockStatus:
      item.stock > 0 ? STOCK_STATUSES.AVAILABLE : STOCK_STATUSES.UNAVAILABLE,
...
export function fromApiInventoryMovement(
  ...
  const card =
    inventoryItem.pokemonCardSummary ?? inventoryItem.magicCardSummary;
  ...
  return {
    ...
    setName:
      inventoryItem.pokemonCardSummary?.setName ??
      inventoryItem.magicCardSummary?.edition ??
      '',
    setCode: inventoryItem.pokemonCardSummary?.setCode ?? '',
    cardNumber:
      inventoryItem.pokemonCardSummary?.cardNumber ??
