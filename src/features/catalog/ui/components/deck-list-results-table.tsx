...
function getStatusChip(line: IDeckListResolvedLine) {
  if (!line.isValid) {
    return (
      <Chip
        ...
        color='danger'
        startContent={
          <Icon icon='lucide:alert-triangle' className='ml-1' width={12} />
        }
      >
        Inválida
      </Chip>
  ...
  if (line.found) {
    return (
      <Chip
        ...
        color='success'
        startContent={
          <Icon icon='lucide:check-circle' className='ml-1' width={12} />
        }
      >
        Encontrada
      </Chip>
  ...
  return (
    <Chip
      ...
      variant='flat'
      color='warning'
      startContent={<Icon icon='lucide:x-circle' className='ml-1' width={12} />}
    >
      No encontrada
    </Chip>
  );
...
function renderCell(
  ...
  switch (columnKey) {
    case 'lineNumber':
      return (
        <span className='text-default-400 text-xs'>{line.lineNumber}</span>
      );
    case 'quantity':
      return (
        <span className='text-sm font-semibold'>
          {line.isValid ? line.quantity : '—'}
        </span>
      );
    case 'cardName':
      return (
        <div className='flex items-center gap-3'>
          ...
          <div className='min-w-0'>
            <p className='truncate text-sm font-medium'>
              {line.cardName || line.raw}
            </p>
            {line.error && (
              <p className='text-danger truncate text-xs'>{line.error}</p>
            )}
          </div>
        </div>
      );
    ...
    case 'stock': {
      ...
      const totalStock = line.card.variants.reduce(
        (sum, v) => sum + v.stock,
        0
      );
    ...
    case 'price': {
      ...
      const lowestPrice = Math.min(
        ...line.card.variants.map((v) => v.sellPrice)
      );
      return (
...
function MobileResultCard({
  ...
  const totalStock =
    line.card?.variants.reduce((sum, v) => sum + v.stock, 0) ?? 0;
  ...
  return (
    <KidstopCard>
      <CardBody className='flex flex-row gap-3 !p-4'>
        {line.card?.imageUrl && (
          <div className='bg-default-100 relative h-14 w-10 flex-shrink-0 overflow-hidden rounded'>
            <Image
              src={line.card.imageUrl}
              alt={line.cardName}
              fill
        ...
        <div className='flex min-w-0 flex-1 flex-col gap-1'>
          ...
          {line.error && <p className='text-danger text-xs'>{line.error}</p>}
