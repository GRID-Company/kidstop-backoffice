...
export function useItemsReferencePrices(
  items: IPurchaseItem[]
): UseItemsReferencePricesReturn {
  ...
  const _pokemonCardGuids = useMemo(
    () => [...new Set(pokemonItems.map((i) => i.cardGuid))],
    [pokemonItems]
  );
  const _magicCardGuids = useMemo(
  ...
  const refetch = useCallback(
    (itemsToRefetch?: IPurchaseItem[]) => {
      const itemsForRefetch = itemsToRefetch || items;
      const pokemonGuids = [
        ...new Set(
          itemsForRefetch
            .filter((i) => i.tcgType === TCG_TYPES.POKEMON)
            .map((i) => i.cardGuid)
        ),
      ];
      const magicGuids = [
        ...new Set(
          itemsForRefetch
            .filter((i) => i.tcgType === TCG_TYPES.MAGIC)
            .map((i) => i.cardGuid)
        ),
      ];
        ...
        for (const guid of pokemonGuids) {
          ...
          try {
            ...
            if (error instanceof Error && error.name !== 'AbortError') {
              console.warn(
                `Failed to refetch Pokemon price for ${guid}:`,
                error
              );
            }
          }
