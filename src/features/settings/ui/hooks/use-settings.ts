...
import {
  UploadFileDocument,
  GetBannerDocument,
} from '@/lib/api/generated/files.generated';
import {
  IGeofenceConfig,
  IOperatingHours,
  IThresholdConfig,
...
export function useSettings() {
  ...
  const { data: buyersData } = useQuery(UsersDocument, {
    variables: {
      findUsersArgs: {
        ...
        filters: {
          role: { filterType: ':multiple_values:', values: ['BUYER'] },
        },
      },
    },
    fetchPolicy: 'cache-and-network',
  ...
  const settings = configData
    ? fromApiToSettings(configData.globalConfig)
    : null;
    ...
    async (geofence: IGeofenceConfig) => {
      try {
        await updateConfigMutation({
          variables: {
            updateGlobalConfigInput: toGeofenceMutationInput(geofence),
          },
        });
  ...
  const updateThresholds = useCallback(
    async (thresholds: IThresholdConfig) => {
      try {
        await updateConfigMutation({
          variables: {
            updateGlobalConfigInput: toThresholdsMutationInput(thresholds),
          },
        });
        ...
        await updateConfigMutation({
          variables: {
            updateGlobalConfigInput:
              toOperatingHoursMutationInput(operatingHours),
          },
        });
    ...
    async (form: BudgetFormData) => {
      try {
        await updateBudgetMutation({
          variables: toUpdateBuyerBudgetPayload(form),
        });
  ...
  const updateBanners = useCallback(
    async (pokemonFile?: File, magicFile?: File) => {
      try {
        ...
        await updateConfigMutation({
          variables: {
            updateGlobalConfigInput: toBannerMutationInput(bannerGuids),
          },
        });
