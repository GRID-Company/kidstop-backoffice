...
The Purchases API manages the business's card acquisition workflow — buying cards from sellers. Purchases follow a strict status machine (DRAFT → QUOTED → WAITING\_PRICE → FINALIZED) and track seller information for each transaction. All endpoints require authentication and use GraphQL.
...
4. **Seller accepts** → transition QUOTED → WAITING\_PRICE
