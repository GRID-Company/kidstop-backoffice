...
The Purchases API manages the business's card acquisition workflow — buying cards from sellers. Purchases follow a strict status machine (DRAFT → QUOTED → WAITING_PRICE → FINALIZED) and track seller information for each transaction. All endpoints require authentication and use GraphQL.
...
QUOTED → WAITING_PRICE  (ADMIN/BUYER — seller accepted quotation)
QUOTED → REJECTED       (ADMIN/BUYER — seller rejected quotation)
...
**Search:** Searches across purchase reference, seller name, seller phone, seller email, and item card names
...
```json
{
  "createPurchaseInput": {
    ...
    "notes": "Purchase from regular seller",
    "items": [
      {
...
**Variables (without seller):**

```json
{
  "createPurchaseInput": {
    "tcg": "POKEMON",
    "notes": "Purchase without seller information",
    "items": [
      {
        "pokemonCardGuid": "CARD_GUID",
        "condition": "NEAR_MINT",
...
- `sellerGuid`: Optional — GUID of a seller from the Sellers API
- `notes`: Optional
...
- `sellerGuid` is optional — purchases can be created without seller information
...
**Description:** Update purchase details (seller info, notes, payments). Cannot modify items through this endpoint.
...
- Seller/notes: editable on DRAFT or QUOTED only
- Payments: editable at any stage except FINALIZED or REJECTED
- Setting `sellerGuid` to null removes the seller association
...
**Description:** Finalize a purchase (WAITING_PRICE → FINALIZED), creating inventory entries and stock movements
**Access:** ADMIN, BUYER
...
1. **Create** a purchase as DRAFT with items and seller info (optional)
2. **Edit items** if needed (only while DRAFT)
3. **Transition** DRAFT → QUOTED (assigns buyer)
4. **Seller accepts** → transition QUOTED → WAITING_PRICE
5. **Set sell prices** on each item during WAITING_PRICE
