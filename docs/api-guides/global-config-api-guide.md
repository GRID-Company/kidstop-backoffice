...
- `config.bannerGuids`: Banner file GUIDs for Pokemon and Magic TCGs
...
```json
{
  "updateGlobalConfigInput": {
    ...
    "bannerGuids": {
      "pokemon": "abc-123-pokemon-banner-file-guid",
      "magic": "xyz-789-magic-banner-file-guid"
    }
  }
}
```
...
- `bannerGuids`: (optional) Banner file GUIDs for each TCG
  - `pokemon`: File GUID for Pokemon banner image
  - `magic`: File GUID for Magic banner image
...
- `bannerGuids` stores file GUIDs that reference uploaded banner images; files must be uploaded via the Files API first
- Banner file GUIDs are validated to ensure they exist before saving
...
**Query:** `getBanner`
**Description:** Retrieve the banner file for a specific TCG (Pokemon or Magic). Returns the complete File entity with URL and metadata. This endpoint is public (no authentication required).
**Roles:** Public (no authentication required)

```graphql
query GetBanner($tcg: TCGType!) {
  getBanner(tcg: $tcg) {
    guid
    name
    path
...
- `tcg`: (required) TCG type - either `POKEMON` or `MAGIC`

**Response Fields:**

- `guid`: File unique identifier
- `name`: File name
- `path`: Full S3 URL to the banner image
- `category`: File category (e.g., `TECHNICAL_IMAGE`)
- `createdDate`: File upload timestamp
- `updatedDate`: Last update timestamp
...
```json
{
  "data": {
    "getBanner": {
      ...
      "name": "Pokemon Banner",
      "path": "https://s3.amazonaws.com/bucket/pokemon-banner.jpg",
...
- Returns `null` if no banner is configured for the specified TCG
- Throws `NotFoundException` if a banner GUID is configured but the file no longer exists
- No authentication required - suitable for public-facing pages
- Use the `path` field to display the banner image in your UI

**Usage Flow:**

1. Admin uploads banner image via Files API → receives file GUID
2. Admin updates global config with banner GUID via `updateGlobalConfig`
3. Frontend calls `getBanner(tcg: POKEMON)` to retrieve banner URL
4. Display banner using the `path` field from response
